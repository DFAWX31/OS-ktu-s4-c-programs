...
typedef struct {
	int process_id, arrival_time, burst_time, priority, waiting_time, turnaround_time;
} program;
...
void main() {
	...
	for (int i = 0; i < num; i++) {
		...
		printf("Enter the priority, arrival time and burst time of process number %d: ", i + 1);
		scanf("%d %d %d", &programs[i].priority, &programs[i].arrival_time, &programs[i].burst_time);
	}
	...
	printf("pid\tp\tat\tbt\twt\ttt\n");
	for (int i = 0; i < num; i++) {
		total_waiting_time += programs[i].waiting_time;
		total_turnaround_time += programs[i].turnaround_time;

		printf("%d\t%d\t%d\t%d\t%d\t%d\n", programs[i].process_id, programs[i].priority, programs[i].arrival_time, programs[i].burst_time, programs[i].waiting_time, programs[i].turnaround_time);
	}
...
void insert_into_ready_queue(int *array, int *size, int element_number, program *programs) {
	...
	for (int i = 0; i <= *size; i++) {
		if (programs[array[i]].priority > programs[element_number].priority) {
