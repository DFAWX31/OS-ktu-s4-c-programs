...
void increase_page_age(int *, int, int, int);
...
void main() {
  ...
  for (int i = 0; i < number_of_processes; i++) {
    if (check_page_hit(page_sequence[i - 1], i < page_size ? i : page_size, process_id[i])) {
      ...
      increase_page_age(page_age, page_size, i, 1);
      page_status[i] = 0;
    } else {
      ...
      increase_page_age(page_age, page_size, oldest_page, 0);
      page_status[i] = 1;
    }
  }
...
void increase_page_age(int *array, int size, int enter_at, int flag) {
  for (int i = 0; i < size; i++) {
    if (flag == 0 && i == enter_at) {
      array[enter_at] = 1;
    } else {
      array[i] += 1;
    }
